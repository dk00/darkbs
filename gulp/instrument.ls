require! \cli-color
{read-file-sync, writeFileSync: writeFile} = require \fs
...
export function instrument(opts)
  through (file, encoding, callback) ->
    ...
    instrumenter = new LivescriptInstrumenter {coverage-variable}
...
export function hook-require(opts)
  through do
...
# write-reports :: Opts -> (? -> ?)
export function write-reports({log-summary}:opts?)
  through do
